  ...
  return (
    <>
      <ActionButton onClick={() => setOpen(true)}>
        <Pencil size={18} />
        <span className="text-[10px] opacity-80">Изменить</span>
      </ActionButton>
        ...
        <Drawer.Portal>
          ...
          <Drawer.Content className="fixed bottom-0 left-0 right-0 rounded-t-2xl bg-neutral-900 text-white p-4">
            <div className="mx-auto max-w-md">
              ...
              <div className="text-sm font-medium mb-2">Редактировать текст</div>
              <textarea
              ...
              <div className="flex justify-end gap-2 mt-3">
                <button onClick={() => setOpen(false)} className="px-3 py-1.5 text-sm rounded-xl bg-white/10">Отмена</button>
                <button onClick={save} className="px-3 py-1.5 text-sm rounded-xl bg-white text-black">Сохранить</button>
              </div>
            </div>
          </Drawer.Content>
        </Drawer.Portal>
